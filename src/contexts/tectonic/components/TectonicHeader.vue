...
const difficultyLabel = computed(() => {
  const config = TectonicGenerator.getDifficultyConfig(store.difficulty)
  const size = `${config.rows}x${config.cols}`
  switch (store.difficulty) {
    case TectonicDifficulty.FACILE:
      return `Facile (${size})`
    case TectonicDifficulty.MOYEN:
      return `Moyen (${size})`
    case TectonicDifficulty.DIFFICILE:
      return `Difficile (${size})`
    case TectonicDifficulty.EXPERT:
      return `Expert (${size})`
    default:
      return 'Facile'
  }
...
<template>
  <div class="tectonic-header">
    <div class="header-top">
      <h1 class="title">Tectonic</h1>
      <div class="timer">{{ store.formattedTime }}</div>
    </div>

    <div class="header-info">
      <div class="info-item">
        <span class="label">Difficulte:</span>
    ...
    <div class="header-actions">
      <button v-if="!store.isPaused && !store.isCompleted" class="header-btn" @click="store.pauseGame">
        Pause
      </button>
    ...
    <div v-if="store.isCompleted" class="completion-message">
      <h2>Felicitations !</h2>
      <p>Puzzle termine en {{ store.formattedTime }}</p>
    ...
    <div v-if="store.isPaused" class="pause-overlay">
      ...
      <button class="header-btn primary resume-btn" @click="store.resumeGame">Reprendre</button>
    </div>
  </div>
</template>
