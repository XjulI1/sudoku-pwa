...
const difficultyLabel = computed(() => {
  const cells = store.maxValue
  switch (store.difficulty) {
    case RikudoDifficulty.FACILE:
      return `Facile (${cells} cases)`
    case RikudoDifficulty.MOYEN:
      return `Moyen (${cells} cases)`
    case RikudoDifficulty.DIFFICILE:
      return `Difficile (${cells} cases)`
    case RikudoDifficulty.EXPERT:
      return `Expert (${cells} cases)`
    default:
      return 'Facile'
  }
})
...
<template>
  <div class="rikudo-header">
    <div class="header-top">
      <h1 class="title">Rikudo</h1>
