...
import type { Grid, Position } from '@/contexts/sudoku/types/sudoku'
import { Difficulty, GridSize } from '@/contexts/sudoku/types/sudoku'
import { SudokuGenerator } from '@/contexts/sudoku/utils/sudokuGenerator'
import { SudokuValidator } from '@/contexts/sudoku/utils/sudokuValidator'
  ...
  function checkCompletion() {
    if (SudokuValidator.isFilled(grid.value)) {
      if (SudokuValidator.isComplete(grid.value, solution.value)) {
        ...
        SudokuStatsManager.saveGameStats(
          difficulty.value,
          gridSize.value,
          elapsedTime.value,
          errorsCount.value,
  ...
  function loadGame() {
    ...
    try {
      ...
      grid.value = state.grid.map((row: unknown[]) =>
        (row as Array<{ notes: number[] }>).map((cell) => ({
          ...cell,
