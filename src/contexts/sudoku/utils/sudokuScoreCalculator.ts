import { Difficulty } from '@/contexts/sudoku/types/sudoku'

/**
 * Calcule la note d'une partie sur 10
 * Pondération: erreur > indice > temps > pause
 * Les notes n'influent pas sur la note finale
 */
export class SudokuScoreCalculator {
  // Temps de référence par difficulté (en millisecondes)
  private static readonly REFERENCE_TIMES: Record<Difficulty, number> = {
    [Difficulty.SIMPLE]: 5 * 60 * 1000, // 5 minutes
    [Difficulty.NORMAL]: 10 * 60 * 1000, // 10 minutes
    [Difficulty.EXPERT]: 20 * 60 * 1000, // 20 minutes
    [Difficulty.MAITRE]: 30 * 60 * 1000, // 30 minutes
    [Difficulty.DIEUX_SUDOKU]: 45 * 60 * 1000 // 45 minutes
  }
   ...
   */
  private static calculateTimeScore(completionTime: number, difficulty: Difficulty): number {
